...
class LockJoints:
    def __init__(self, index=None, norm='l2', weights=None):
        self.index = list(range(51, 63)) if index is None else index
        self.norm  = norm
        self._index_tensor = None

    def _get_index_tensor(self, device):
        if self._index_tensor is None or self._index_tensor.device != device:
            import torch
            self._index_tensor = torch.tensor(self.index, dtype=torch.long, device=device)
        return self._index_tensor

    def __call__(self, poses, init_poses, **kwargs):
        import torch

        # DEBUG — xóa sau khi fix xong
        print("=== LockJoints DEBUG ===")
        print(f"  poses      type={type(poses)}", end="")
        if isinstance(poses, dict):
            print(f"  keys={list(poses.keys())}")
        else:
            print(f"  shape={poses.shape}")

        print(f"  init_poses type={type(init_poses)}", end="")
        if isinstance(init_poses, dict):
            print(f"  keys={list(init_poses.keys())}")
        else:
            print(f"  shape={init_poses.shape}")
        print("========================")

        # unwrap dict — dùng key thực tế in ra ở trên
        if isinstance(poses, dict):
            key = list(poses.keys())[0]
            poses = poses[key]
        if isinstance(init_poses, dict):
            key = list(init_poses.keys())[0]
            init_poses = init_poses[key]

        idx = self._get_index_tensor(poses.device)
        diff = poses[..., idx] - init_poses[..., idx]
        loss = (diff ** 2).mean()
        return loss
