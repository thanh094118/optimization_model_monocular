...
class MultiStage:
    ...
    def __init__(self, output, at_step, at_final, keys_keep=[], timer=True, root=None, **kwargs) -> None:
        # Tự động tạo output path từ root
        if root is not None:
            # Lấy tên thư mục cuối cùng từ root path
            folder_name = os.path.basename(os.path.abspath(root))
            
            # Loại bỏ 'output/' prefix nếu có trong output
            if output.startswith('output/'):
                output = output[7:]  # Bỏ 'output/'
            
            # Tạo path mới: output/camera1/sv1p
            output = os.path.join('output', folder_name, output)
